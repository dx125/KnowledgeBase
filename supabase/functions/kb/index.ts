...
//
// The ONLY component that touches the database. The browser calls these HTTP
// endpoints; it never sees Postgres/PostgREST. The function authenticates to the
// DB with the service_role key (injected by Supabase, never shipped to clients)
// and delegates to the SQL RPCs defined in the migrations.
//
// Authorization
// -------------
// Every data endpoint requires a logged-in user (Supabase Auth email+password).
// The caller must send the user's access token:  Authorization: Bearer <token>.
// The project anon key alone (no user) is rejected with 401. Only `GET /` is open.
//
...
//   GET  /topics/:topicId/cards  → all cards in a topic
//   GET  /search                 → ranked search (?q, ?topic, ?limit, ?offset)
//   GET  /cards/:cardId          → single card (incl. keywords/subtopics)
...
Deno.serve(async (req) => {
  ...
  if (segments.length === 0 && req.method === 'GET') {
    return json({
      ...
      version: 1,
      auth: 'Bearer <user access token> required on all routes except GET /',
      default_locale: DEFAULT_LOCALE,
      supported_locales: SUPPORTED_LOCALES,
      endpoints: {
        me: 'GET /me · PUT /me {default_locale}',
        topics: 'GET /topics?locale&internal',
        topic_cards: 'GET /topics/:topicId/cards?locale&internal',
  ...
  try {
    ...
    if (segments.length === 3 && segments[0] === 'topics' && segments[2] === 'cards') {
      ...
      if (error) throw error;
      return json({ locale, topic_id: topicId, include_internal: includeInternal, count: data?.length ?? 0, cards: data ?? [] });
    }
